...
local function get_auto_cycle_status(cycle, running, additional_info)
	return S("Cycles remaining: @1", cycle)
		.. (additional_info and ("\n" .. additional_info) or "")
		.. "\n" .. (running and running_msg or S("Halted!"))
end
...
auto_cycle_emerge = function(pos)
	...
	core.emerge_area(min, max, function(_, action, remaining)
		...
		if remaining <= 0 then
			...
			core.after(math.max(period - elapsed, 0), auto_cycle_dig, pos)
		end
	end)
end

-- The actual dig and repeat of auto cycling. Repeating happens via the `auto_cycle_emerge`
-- and `check_continue` functions. (Symbol `auto_cycle_dig` declared local above.)
auto_cycle_dig = function(pos)
	local meta = core.get_meta(pos)
	local player = core.get_player_by_name(meta:get_string("triggering_player"))
	...
	if meta:get_string("waiting") == "true" then
		-- Formspec and status message is managed by `auto_cycle_interrupt`.
		return
	end
...
local function auto_cycle_interrupt(pos)
	...
	meta:set_string("formspec", auto_formspec)
	-- If `running_msg` (which is always last) is found in infotext, replace it.
	local status = meta:get_string("infotext")
	local msg_pos = status:find(running_msg, 1, true)
	if msg_pos then
